...
public interface AnnotationRepository extends JpaRepository<Annotation, Long>, JpaSpecificationExecutor<Annotation> {
    ...
    @Query(value = "select distinct annotation from Annotation annotation left join fetch annotation.dataRecords",
    ...
    @Query("select annotation from Annotation annotation left join fetch annotation.dataRecords where annotation.id =:id")
