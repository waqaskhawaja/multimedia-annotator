...
public class AnnotationResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllAnnotationsByDataRecordIsEqualToSomething() throws Exception {
        // Initialize the database
        DataRecord dataRecord = DataRecordResourceIntTest.createEntity(em);
        em.persist(dataRecord);
        em.flush();
        annotation.addDataRecord(dataRecord);
        annotationRepository.saveAndFlush(annotation);
        Long dataRecordId = dataRecord.getId();

        // Get all the annotationList where dataRecord equals to dataRecordId
        defaultAnnotationShouldBeFound("dataRecordId.equals=" + dataRecordId);

        // Get all the annotationList where dataRecord equals to dataRecordId + 1
        defaultAnnotationShouldNotBeFound("dataRecordId.equals=" + (dataRecordId + 1));
    }
