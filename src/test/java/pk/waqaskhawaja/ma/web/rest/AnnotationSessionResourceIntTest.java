...
public class AnnotationSessionResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllAnnotationSessionsByAnalysisSessionIsEqualToSomething() throws Exception {
        // Initialize the database
        AnalysisSession analysisSession = AnalysisSessionResourceIntTest.createEntity(em);
        em.persist(analysisSession);
        em.flush();
        annotationSession.setAnalysisSession(analysisSession);
        annotationSessionRepository.saveAndFlush(annotationSession);
        Long analysisSessionId = analysisSession.getId();

        // Get all the annotationSessionList where analysisSession equals to analysisSessionId
        defaultAnnotationSessionShouldBeFound("analysisSessionId.equals=" + analysisSessionId);

        // Get all the annotationSessionList where analysisSession equals to analysisSessionId + 1
        defaultAnnotationSessionShouldNotBeFound("analysisSessionId.equals=" + (analysisSessionId + 1));
    }
