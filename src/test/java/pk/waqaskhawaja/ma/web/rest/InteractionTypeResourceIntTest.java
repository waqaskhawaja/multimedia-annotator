...
public class InteractionTypeResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllInteractionTypesByInteractionRecordIsEqualToSomething() throws Exception {
        // Initialize the database
        InteractionRecord interactionRecord = InteractionRecordResourceIntTest.createEntity(em);
        em.persist(interactionRecord);
        em.flush();
        interactionType.addInteractionRecord(interactionRecord);
        interactionTypeRepository.saveAndFlush(interactionType);
        Long interactionRecordId = interactionRecord.getId();

        // Get all the interactionTypeList where interactionRecord equals to interactionRecordId
        defaultInteractionTypeShouldBeFound("interactionRecordId.equals=" + interactionRecordId);

        // Get all the interactionTypeList where interactionRecord equals to interactionRecordId + 1
        defaultInteractionTypeShouldNotBeFound("interactionRecordId.equals=" + (interactionRecordId + 1));
    }
